package com.linkhand.baixingguanjia.base.adapter;
...
//            videoView.start();
    ...
    public void setImage(int resId, String path) {
        ...
        Glide.with(mContext).load(path).asBitmap().placeholder(R.drawable.touxiang).error(R.drawable.touxiang).centerCrop().into(new BitmapImageViewTarget(img) {
            @Override
    ...
    public void setImageVoideo(int resId, String path) {
        final ImageView img = getSubView(resId);
        Glide.with(mContext).load(path).asBitmap().placeholder(R.color.colorBorder).error(R.color.colorBorder).centerCrop().into(new BitmapImageViewTarget(img) {
            @Override
            protected void setResource(Bitmap resource) {
                RoundedBitmapDrawable circularBitmapDrawable =
                        RoundedBitmapDrawableFactory.create(mContext.getResources(), resource);
    ...
    //该方法设置的默认图片的大图的默认图片
    public void setImagePhoto(int resId, String path) {
        final ImageView img = getSubView(resId);
        Glide.with(mContext).load(path).asBitmap().placeholder(R.drawable.login_tupian_def).error(R.drawable.login_tupian_def).into(new BitmapImageViewTarget(img) {
            @Override
            protected void setResource(Bitmap resource) {
                RoundedBitmapDrawable circularBitmapDrawable =
                        RoundedBitmapDrawableFactory.create(mContext.getResources(), resource);
