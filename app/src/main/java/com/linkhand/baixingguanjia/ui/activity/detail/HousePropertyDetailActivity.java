...
public class HousePropertyDetailActivity extends BaseActivity implements OnItemClickListener, ViewPager.OnPageChangeListener {
...
//    private static final int REQUEST_WHAT = 1;
    ...
    @Bind(R.id.house_huxing)
    TextView mHouseHuxingTV;
    @Bind(R.id.house_mianji)
    TextView mHouseMianjiTV;
    ...
    protected void getBundleExtras(Bundle extras) {
        ...
        if (extras != null) {
            mHouse = (HouseProperty) extras.getSerializable("house");
            if (mHouse != null) {
                if (adjustList(mHouse.getImage_url())) {
    ...
    private void initDialog() {
        ...
        noBtn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                //1下线2不下线
                if (mHouse.getOffline().equals("1")) {
                    if (position != -1){
                        EventBus.getDefault().post(new EventFlag("offlineHouse",position));
                    }
                }
                finish();
            }
...
//                    initListener();
    ...
    private void setViewData() {
        mHouseNameTV.setText(mHouse.getTitle());
        ...
        mReleaseTimeTV.setText(DateTimeUtils.formatdian(mHouse.getAdd_time()));
        // 1出租2出售【默认2】
        if (mHouse.getHost_type().equals("1")) {
            mHouseTypeTV.setText(R.string.rentPrice);
            mPriceTV.setText(mHouse.getPrice() + "元/月");
            mHousePriceTV.setText(mHouse.getPrice() + "元/月");
        } else {
            mHouseTypeTV.setText(R.string.sellPrice);
            mPriceTV.setText(mHouse.getPrice() + "元/平");
            mHousePriceTV.setText(mHouse.getPrice() + "元/平");
        }
        mContentTV.setText(mHouse.getContent());


        mHouseMianjiTV.setText(mHouse.getArea() + ""); //面积
        if (!TextUtils.isEmpty(mHouse.getShi())) {
            mHouseHuxingTV.setText(mHouse.getShi() + "室");
            if (!TextUtils.isEmpty(mHouse.getTing())) {
                mHouseHuxingTV.setText(mHouse.getShi() + "室" + mHouse.getTing() + "厅");
                if (!TextUtils.isEmpty(mHouse.getChu())) {
                    mHouseHuxingTV.setText(mHouse.getShi() + "室" + mHouse.getTing() + "厅" + mHouse.getChu() + "厨");
                    if (!TextUtils.isEmpty(mHouse.getWei())) {
                        mHouseHuxingTV.setText(mHouse.getShi() + "室" + mHouse.getTing() + "厅" + mHouse.getChu() + "厨" + mHouse.getChu() + "卫");
                    }
                }
                if (!TextUtils.isEmpty(mHouse.getWei())) {
                    mHouseHuxingTV.setText(mHouse.getShi() + "室" + mHouse.getTing() + "厅" + mHouse.getChu() + "卫");
                }
            }
            if (!TextUtils.isEmpty(mHouse.getChu())) {
                mHouseHuxingTV.setText(mHouse.getShi() + "室" + mHouse.getChu() + "厨");
                if (!TextUtils.isEmpty(mHouse.getWei())) {
                    mHouseHuxingTV.setText(mHouse.getShi() + "室" + mHouse.getChu() + "厨" + mHouse.getChu() + "卫");
                }
            }
            if (!TextUtils.isEmpty(mHouse.getWei())) {
                mHouseHuxingTV.setText(mHouse.getShi() + "室" + mHouse.getChu() + "卫");
            }
        }


        mPhoneTV.setText("******");
        mFuwuTypeTV.setText(R.string.housetype);
        if (mHouse.getCategory().equals("4")) {
            mStoreTypeTV.setText(R.string.store);
        } else if (mHouse.getCategory().equals("5")) {
            mStoreTypeTV.setText(R.string.geren);
        ...
        mAddressTV.setText(mHouse.getAddress());
        mContactTV.setText(mHouse.getBody());
    }
...
//        } else { }
                ...
                if (what == HTTP_REQUEST_IS_AAP) {// 根据what判断是哪个请求的返回，这样就可以用一个OnResponseListener来接受多个请求的结果。
                    ...
                    try {
                        if (jsonObject.getString("code").equals("200")) {
                            //已预约
                            mPhoneTV.setText(mHouse.getPhone());
    ...
    private void httpAppointment() {
        ...
        request.add("goodsid", mHouse.getId());
        request.add("goodstype", mHouse.getGoods_type());
        request.add("offline", mHouse.getOffline());
        request.add("userid", MyApplication.getUser().getUserid());
