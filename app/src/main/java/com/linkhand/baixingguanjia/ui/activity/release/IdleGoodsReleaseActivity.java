...
public class IdleGoodsReleaseActivity extends BaseActivity {
    ...
    @Bind(R.id.jichengxin_edit)
    EditText mJichengxinEdit;
    @Bind(R.id.maijia_price)
    EditText mMaijiaPrice; //想买多少
    @Bind(R.id.gouru_price)
    EditText mGouruPrice;
    ...
    String company = "";
    int jichengxin;
    double gouruPrice, sellPrice; // 购入/售卖价格
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_idle_goods_release);
        ...
        mGridView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> arg0, View arg1, int position, long arg3) {
                ...
                if (imgPaths.get(position).equals("")) {
                    ...
                    //开始预览
                    Intent intent = new Intent(IdleGoodsReleaseActivity.this, MultiPhotoViewActivity.class);
...
//                    initListener();
    ...
    private void setViewData() {
        ...
        categoryStr = mIdleGoods.getCategory();
        mCommonType = new CommonType(mIdleGoods.getClassify(), mIdleGoods.getLeibie());
        mGoodsType.setText(mIdleGoods.getLeibie());//
        mTitleET.setText(mIdleGoods.getTitle()); //标题
        mDescribeTV.setText(mIdleGoods.getContent()); //描述
        mJichengxinEdit.setText(mIdleGoods.getSeveral()); //
        mMaijiaPrice.setText(mIdleGoods.getSales_price());
        mGouruPrice.setText(mIdleGoods.getBuy_price());
        mContactET.setText(mIdleGoods.getContact());
        mPhoneET.setText(mIdleGoods.getPhone());
        mAddressTV.setText(mIdleGoods.getCounty() + mIdleGoods.getVillage());
        mCompanyNameTV.setText(mIdleGoods.getCompany());
        shengId = mIdleGoods.getSheng();
        shiId = mIdleGoods.getShi();
        quId = mIdleGoods.getQu();
        xiaoquId = mIdleGoods.getXiaoqu();
        imgPaths.clear();
    ...
    public void onViewClicked(View view) {
        switch (view.getId()) {
            ...
            case R.id.sort_layout:
                ...
                bundle.putString("flag", "idletype");
                go(CommonSelectedTypeActivity.class, bundle);
                break;
            case R.id.location_layout:
                Bundle bundleP = new Bundle();
                bundleP.putString("acFlag", "IdleRelease");
...
//                onAddress2Picker();
            ...
            case R.id.abstract_layout:
                ...
                bundle1.putString("flag", "idleDescribe");
    ...
    private void judgeData() {
        ...
        if (TextUtils.isEmpty(mJichengxinEdit.getText().toString())) {
            jichengxin = 0;
        } else
            jichengxin = Integer.parseInt(mJichengxinEdit.getText().toString());
        if (TextUtils.isEmpty(mMaijiaPrice.getText().toString())) {
            sellPrice = 0;
        } else
            sellPrice = Double.parseDouble(mMaijiaPrice.getText().toString());
        if (TextUtils.isEmpty(mGouruPrice.getText().toString())) {
            gouruPrice = 0;
        } else
            gouruPrice = Double.parseDouble(mGouruPrice.getText().toString());
        ...
        if (jichengxin <= 0) {//几成新
            showToast(R.string.toastjichengxin);
            return;
        }
        if (sellPrice <= 0) {//想卖多少
        ...
        if (gouruPrice <= 0) {//购入价格
            showToast(R.string.toastBuyPrice);
            return;
        }
...
//                httpUploadInfo();
    ...
    private void httpUploadInfo() {
        ...
        if (!flag.contains("update")) {
            request = NoHttp.createJsonObjectRequest(ConnectUrl.PUBLIC_IDLE_GOODS_RELEASE, RequestMethod.POST);
        ...
        request.add("data[buy_price]", gouruPrice);
        request.add("data[category]", categoryStr);//类型4商家5个人
        request.add("data[sales_price]", sellPrice);//
        request.add("data[several]", jichengxin);//  几成新

        request.add("data[company]", company);
        request.add("data[classify]", mCommonType.getId());
...
//                showToast(R.string.uploadFail);
    ...
    private void emptyView() {
        ...
        mTitleET.setText("");
        mDescribeTV.setText("");
        mJichengxinEdit.setText("");
        mMaijiaPrice.setText("");
        mGouruPrice.setText("");
        mContactET.setText("");
        mPhoneET.setText("");
        mAddressTV.setText("");
    ...
    private void initSelectPicView() {
        if (imgPaths.size() == 1 && imgPaths.get(0).equals("")) {
            mPicBg.setBackground(getResources().getDrawable(R.drawable.idle_bg));
...
//                horizontal_layout(imgPaths, mGridView);
    ...
    private void showPaySuccessDialog() {
        final Dialog mDialog = new Dialog(IdleGoodsReleaseActivity.this, R.style.Dialog);
